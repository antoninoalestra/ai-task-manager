...
export async function updateTaskOrEvent(id, updates) {
  ...
  if (typeof updates.reminder_sent === 'boolean') {
    payload.reminder_sent = updates.reminder_sent;
  } else if (updates.start_time) {
    // Se l'orario d'inizio viene aggiornato, ripristina il reminder a false
...
/**
 * Recupera tutti gli eventi imminenti (start_time tra ora e ora+15min)
 * che non sono stati ancora notificati.
 */
export async function getPending15MinReminders() {
  const now = new Date();
  // Tolleranza: include eventi a partire da 2 min prima fino a 15 min nel futuro
  ...
  if (error) throw new Error(`Errore recupero reminder imminenti: ${error.message}`);
  if (!data) return [];
...
/**
 * Marca un evento come notificato impostando reminder_sent = true
 */
export async function markReminderSent(id) {
  ...
  if (error) throw new Error(`Errore aggiornamento reminder_sent: ${error.message}`);
  return data;
}
