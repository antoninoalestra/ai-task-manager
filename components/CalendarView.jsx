...
function MobileAgendaView({ items, onToggleComplete, onEditItem, onDeleteItem, onAddNewItem }) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between bg-[#232730] border border-white/[0.08] rounded-2xl p-2 shadow-lg">
        <button
          type="button"
          onClick={() => navigateDay(-1)}
          aria-label="Giorno precedente"
          className="flex items-center justify-center min-w-[44px] min-h-[44px] rounded-xl bg-[#191c22] text-slate-300 hover:text-white border border-white/[0.06] active:scale-95 transition-all touch-manipulation"
        >
          <ChevronLeft className="w-5 h-5" />
        </button>

        <div className="flex flex-col items-center">
          <span className="text-xs font-bold text-slate-100 capitalize">{selectedDateFormatted}</span>
      ...
      <div className="flex items-center justify-between gap-1 overflow-x-auto no-scrollbar p-1.5 bg-[#16181d] border border-white/[0.06] rounded-2xl">
        {daysStrip.map((day) => (
          <button
            key={day.dateStr}
            type="button"
            ...
            className={`flex-1 min-w-[44px] min-h-[48px] py-2 px-1 rounded-xl flex flex-col items-center justify-center transition-all touch-manipulation ${
              day.isSelected
                ? 'bg-indigo-600 text-white font-bold shadow-md shadow-indigo-600/25 scale-105'
                : day.isToday
                ? 'bg-indigo-950/60 text-indigo-300 font-bold border border-indigo-800/40'
                : 'text-slate-400 hover:text-white bg-[#232730]'
            }`}
          >
      ...
      <div className="flex items-center justify-between px-1">
        ...
        <button
          ...
          className="min-h-[40px] px-3.5 py-1.5 rounded-xl bg-indigo-600/90 hover:bg-indigo-600 text-white text-xs font-semibold transition-all flex items-center gap-1.5 shadow-md shadow-indigo-600/20 active:scale-95 touch-manipulation"
        >
      ...
      {dayItems.length === 0 ? (
        <div className="bg-[#232730] border border-white/[0.08] rounded-2xl p-8 text-center text-slate-400 text-xs space-y-2 shadow-xs">
        ...
        <div className="space-y-2">
          {dayItems.map((item) => {
            ...
            if (temporalStatus === 'today') {
              containerClasses = `bg-indigo-950/40 border-indigo-500/40 ring-1 ring-indigo-500/30 shadow-md`;
            } else if (temporalStatus === 'past') {
              containerClasses = `bg-[#191c22] border-white/5 opacity-55 text-slate-400`;
            }
            ...
            return (
              <div
                ...
                className={`p-3.5 rounded-xl border transition-all ${containerClasses} ${
                  item.is_completed ? 'opacity-40' : 'hover:border-slate-400 shadow-sm'
                }`}
              >
                    ...
                    <button
                      ...
                      className={`min-w-[36px] min-h-[36px] w-9 h-9 rounded-xl border flex items-center justify-center transition-all shrink-0 active:scale-95 touch-manipulation ${
                        item.is_completed
                          ...
                          : 'border-slate-500 hover:border-white bg-[#191c22]'
                      }`}
                    >
                    ...
                    >
                      <div className="flex items-center gap-2 mb-1 flex-wrap">
                        ...
                        <span className={`px-2 py-0.5 rounded text-[10px] font-mono font-bold uppercase tracking-wider ${
                          isEvent ? 'bg-indigo-950/80 text-indigo-300 border border-indigo-800/50' : 'bg-[#191c22] text-slate-300 border border-white/[0.06]'
                        }`}>
                          {timeString}
                        </span>
...
function InnerCalendar({ events, onEventClick }) {
  const calendar = useCalendarApp({
    ...
    calendars: {
      casa: {
        colorName: 'casa',
        lightColors: { main: '#fb923c', container: '#2b1b12', onContainer: '#fed7aa' },
        darkColors: { main: '#fb923c', container: 'rgba(251, 146, 60, 0.2)', onContainer: '#ffffff' },
      },
      universita: {
        colorName: 'universita',
        lightColors: { main: '#fbbf24', container: '#2b2212', onContainer: '#fde68a' },
        darkColors: { main: '#fbbf24', container: 'rgba(251, 191, 36, 0.2)', onContainer: '#ffffff' },
      },
      lavoro: {
        colorName: 'lavoro',
        lightColors: { main: '#818cf8', container: '#181b38', onContainer: '#a5b4fc' },
        darkColors: { main: '#818cf8', container: 'rgba(129, 140, 248, 0.2)', onContainer: '#ffffff' },
      },
      personale: {
        colorName: 'personale',
        lightColors: { main: '#34d399', container: '#122b22', onContainer: '#a7f3d0' },
        darkColors: { main: '#34d399', container: 'rgba(52, 211, 153, 0.2)', onContainer: '#ffffff' },
      },
      salute: {
        colorName: 'salute',
        lightColors: { main: '#f472b6', container: '#381628', onContainer: '#fbcfe8' },
        darkColors: { main: '#f472b6', container: 'rgba(244, 114, 182, 0.2)', onContainer: '#ffffff' },
      },
      finanze: {
        colorName: 'finanze',
        lightColors: { main: '#c084fc', container: '#2c183b', onContainer: '#ddd6fe' },
        darkColors: { main: '#c084fc', container: 'rgba(192, 132, 252, 0.2)', onContainer: '#ffffff' },
      },
      generico: {
        colorName: 'generico',
        lightColors: { main: '#94a3b8', container: '#212630', onContainer: '#cbd5e1' },
        darkColors: { main: '#94a3b8', container: 'rgba(148, 163, 184, 0.2)', onContainer: '#ffffff' },
      },
    },
...
export default function CalendarView({ items = [], onToggleComplete, onSaveTask, onDeleteTask }) {
  ...
  return (
    <div className="w-full space-y-4 font-sans">
      {/* CONTENITORE UNIFICATO CALENDARIO VISIVO SOFT DARK */}
      <div className="bg-[#232730] border border-white/[0.08] rounded-2xl p-3.5 sm:p-5 shadow-2xl overflow-hidden min-h-[520px]">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4 pb-3 border-b border-white/[0.06]">
              ...
              <div className="flex items-center gap-3">
                <button
                  ...
                  className="text-xs text-slate-300 bg-[#191c22] hover:bg-white/5 px-3 py-1.5 rounded-xl border border-white/[0.06] transition-all flex items-center gap-1.5 font-medium active:scale-95"
                >
                  {showAllDayBar ? <EyeOff className="w-3.5 h-3.5 text-slate-400" /> : <Eye className="w-3.5 h-3.5 text-slate-400" />}
                  <span>{showAllDayBar ? 'Nascondi' : 'Mostra'} Quick Bar ({dayTasks.length})</span>
                </button>
                <span className="text-[10px] text-slate-400 bg-[#191c22] px-2.5 py-1 rounded-lg border border-white/[0.06] font-mono">
                  Europe/Rome
                </span>
              </div>
            </div>

            {/* BARRA EVENTI DEL GIORNO (TUTTO IL GIORNO) SOFT DARK */}
            {showAllDayBar && (
              <div className="mb-4 p-3.5 bg-[#16181d] border border-white/[0.06] rounded-2xl space-y-3 shadow-inner">
                <div className="flex items-center justify-between border-b border-white/[0.06] pb-2.5">
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    <span className="px-2.5 py-0.5 rounded-full text-[10px] bg-indigo-950/60 text-indigo-300 border border-indigo-800/40 font-semibold font-mono">
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      className="text-xs text-white bg-indigo-600/90 hover:bg-indigo-600 px-3 py-1.5 rounded-xl border border-indigo-500/50 transition-all flex items-center gap-1.5 font-bold shadow-md shadow-indigo-600/20 active:scale-95"
                    >
                ...
                {dayTasks.length === 0 ? (
                  <div className="py-2.5 px-3.5 text-slate-400 text-xs flex items-center justify-between bg-[#232730] border border-white/[0.06] rounded-xl">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2 max-h-[160px] overflow-y-auto pr-1">
                    {dayTasks.map((task) => {
                      ...
                      if (temporalStatus === 'today') {
                        taskStyle = `bg-indigo-950/40 border-indigo-500/40 ring-1 ring-indigo-500/30 shadow-md`;
                      } else if (temporalStatus === 'past') {
                        taskStyle = `bg-[#191c22] border-white/5 opacity-55 text-slate-400`;
                      }
                          ...
                          <div className="flex items-center gap-2.5 min-w-0 flex-1">
                            <button
                              ...
                              className={`min-w-[24px] min-h-[24px] w-6 h-6 rounded-lg border flex items-center justify-center transition-all shrink-0 active:scale-95 touch-manipulation ${
                                task.is_completed
                                  ? 'bg-indigo-600 border-indigo-500 text-white'
                                  : 'border-slate-400 hover:border-white bg-[#191c22]'
                              }`}
                            >
