...
/**
 * Componente Agenda Timeline Ad-Hoc per Smartphone (< 640px) Light Theme
 */
    ...
    <div className="space-y-4">
        ...
        <div className="flex flex-col items-center">
          ...
          {selectedDate !== getLocalDateString(new Date()) && (
            <button
              ...
              className="text-[10px] text-blue-600 font-bold mt-0.5 hover:underline active:scale-95"
            >
              Torna ad Oggi
            </button>
          )}
      ...
      <div className="flex items-center justify-between gap-1 overflow-x-auto no-scrollbar p-1.5 bg-slate-100/80 border border-slate-200 rounded-2xl">
        {daysStrip.map((day) => (
          ...
          >
            ...
            {day.hasEvents && (
              <span className={`w-1.5 h-1.5 rounded-full mt-1 ${day.isSelected ? 'bg-white' : 'bg-blue-500'}`}></span>
            )}
          </button>
        ))}
      </div>
      ...
      <div className="flex items-center justify-between px-1">
        ...
        <button
          ...
          className="min-h-[40px] px-3.5 py-1.5 rounded-xl bg-blue-600 hover:bg-blue-700 text-white text-xs font-bold transition-all flex items-center gap-1.5 shadow-md shadow-blue-600/20 active:scale-95 touch-manipulation"
        >
        ...
        <div className="space-y-2">
          {dayItems.map((item) => {
            ...
            if (temporalStatus === 'today') {
              containerClasses = `bg-blue-50/70 border-blue-300 ring-1 ring-blue-500/30 shadow-md`;
            } else if (temporalStatus === 'past') {
              containerClasses = `bg-slate-50/60 border-slate-200 opacity-60`;
            }
                    ...
                    >
                      <div className="flex items-center gap-2 mb-1 flex-wrap">
                        {temporalStatus === 'today' && (
                          <span className="px-1.5 py-0.5 rounded bg-blue-600 text-white text-[9px] font-bold uppercase tracking-wider">
                            OGGI
                          </span>
                        )}
                        <span className={`px-2 py-0.5 rounded text-[10px] font-mono font-bold uppercase tracking-wider ${
                          isEvent ? 'bg-blue-100 text-blue-800 border border-blue-200' : 'bg-slate-100 text-slate-700 border border-slate-200'
                        }`}>
                          {timeString}
                        </span>
...
function InnerCalendar({ events, onEventClick }) {
  const calendar = useCalendarApp({
    ...
    calendars: {
      ...
      lavoro: {
        colorName: 'lavoro',
        lightColors: { main: '#2563eb', container: '#eff6ff', onContainer: '#1d4ed8' },
        darkColors: { main: '#2563eb', container: '#eff6ff', onContainer: '#1d4ed8' },
      },
      personale: {
        colorName: 'personale',
      ...
      finanze: {
        colorName: 'finanze',
        lightColors: { main: '#6366f1', container: '#eef2ff', onContainer: '#4338ca' },
        darkColors: { main: '#6366f1', container: '#eef2ff', onContainer: '#4338ca' },
      },
      generico: {
        colorName: 'generico',
...
export default function CalendarView({ items = [], onToggleComplete, onSaveTask, onDeleteTask }) {
  ...
  return (
    <div className="w-full space-y-4 font-sans">
      {/* CONTENITORE UNIFICATO CALENDARIO VISIVO LIGHT THEME */}
      <div className="bg-white border border-slate-200 rounded-2xl p-3.5 sm:p-5 shadow-sm overflow-hidden min-h-[520px]">
        ...
        {isMobile ? (
          ...
          <>
            ...
            {/* BARRA EVENTI DEL GIORNO (TUTTO IL GIORNO) APPLE LIGHT STYLE */}
            {showAllDayBar && (
              <div className="mb-4 p-3.5 bg-slate-50/80 border border-slate-200 rounded-2xl space-y-3 shadow-xs">
                <div className="flex items-center justify-between border-b border-slate-200/80 pb-2.5">
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    <span className="px-2.5 py-0.5 rounded-full text-[10px] bg-blue-100 text-blue-800 border border-blue-200 font-bold font-mono">
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      className="text-xs text-white bg-blue-600 hover:bg-blue-700 px-3 py-1.5 rounded-xl border border-blue-600 transition-all flex items-center gap-1.5 font-bold shadow-md shadow-blue-600/20 active:scale-95"
                    >
                ...
                {dayTasks.length === 0 ? (
                  <div className="py-2.5 px-3.5 text-slate-500 text-xs flex items-center justify-between bg-white border border-slate-200 rounded-xl">
                    ...
                    <button
                      type="button"
                      onClick={() => handleAddNewItem(new Date().toISOString().split('T')[0])}
                      className="text-[11px] text-blue-600 hover:text-blue-700 font-bold underline underline-offset-2"
                    >
                      + Crea Evento
                    </button>
                  </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2 max-h-[160px] overflow-y-auto pr-1">
                    {dayTasks.map((task) => {
                      ...
                      if (temporalStatus === 'today') {
                        taskStyle = `bg-blue-50/90 border-blue-300 ring-1 ring-blue-500/30 shadow-md`;
                      } else if (temporalStatus === 'past') {
                          ...
                          <div className="flex items-center gap-2.5 min-w-0 flex-1">
                            ...
                            >
                              <div className="flex items-center gap-1.5">
                                {temporalStatus === 'today' && (
                                  <span className="px-1 py-0.2 rounded bg-blue-600 text-white text-[8px] font-bold uppercase tracking-wider">
                                    OGGI
                                  </span>
                                )}
                                <p
