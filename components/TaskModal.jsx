...
export default function TaskModal({ isOpen, onClose, taskToEdit = null, initialDate = '', onSave, onDelete }) {
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-end sm:items-center justify-center bg-slate-900/40 backdrop-blur-sm transition-opacity">
      ...
      <div className="relative w-full max-w-lg bg-white border-t sm:border border-slate-200 rounded-t-3xl sm:rounded-2xl shadow-2xl shadow-slate-900/10 overflow-hidden text-slate-900 max-h-[85dvh] sm:max-h-[90vh] flex flex-col z-10 animate-slide-up-sheet sm:animate-none">
        ...
        <div className="flex items-center justify-between px-5 py-3.5 border-b border-slate-200 bg-slate-50/80">
          <h2 className="text-xs font-bold tracking-wider uppercase text-slate-800 flex items-center gap-2">
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4 text-xs overflow-y-auto flex-1">
          ...
          <div>
            ...
            <input
              ...
              className="w-full px-3.5 py-3 rounded-xl bg-slate-100/80 border border-slate-200 text-slate-900 placeholder-slate-400 focus:outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 transition-all text-xs min-h-[44px] font-medium"
            />
          </div>
          ...
          <div className="relative">
            ...
            <button
              ...
              className="w-full min-h-[44px] px-3.5 py-2.5 rounded-xl bg-slate-100/80 border border-slate-200 hover:border-slate-300 text-slate-900 transition-all text-xs flex items-center justify-between cursor-pointer"
            >
            ...
            {isCategoryDropdownOpen && (
              <>
                ...
                />
                <div className="absolute z-50 left-0 right-0 mt-1 py-1.5 bg-white border border-slate-200 rounded-xl shadow-xl space-y-0.5 max-h-56 overflow-y-auto animate-fade-in">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              <div>
                ...
                <input
                  ...
                  className="w-full min-h-[44px] px-3.5 py-2.5 rounded-xl bg-slate-100/80 border border-slate-200 text-slate-900 focus:outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 transition-all text-xs font-mono font-medium"
                />
              </div>
          ...
          {type === 'event' && (
            <div className="flex items-center gap-1.5 pt-1 overflow-x-auto no-scrollbar">
              <span className="text-[10px] text-slate-500 font-bold uppercase shrink-0">Preset:</span>
              {[
                { label: '15m', min: 15 },
                { label: '30m', min: 30 },
                { label: '45m', min: 45 },
                ...
                <button
                  ...
                  className="min-h-[36px] px-3 py-1.5 rounded-lg bg-slate-100 hover:bg-indigo-50 text-slate-700 hover:text-indigo-700 border border-slate-200 text-[11px] font-bold transition-all shrink-0 active:scale-95"
                >
                  {p.label}
                </button>
              ))}
          ...
          <div>
            ...
            <textarea
              ...
              className="w-full px-3.5 py-2.5 rounded-xl bg-slate-100/80 border border-slate-200 text-slate-900 placeholder-slate-400 focus:outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 transition-all text-xs font-medium"
            />
          </div>

          {/* Pulsanti Azione */}
          <div className="flex items-center justify-between pt-4 border-t border-slate-200 mt-4 pb-safe">
            {taskToEdit ? (
              <button
                type="button"
                onClick={handleDelete}
                disabled={isSubmitting}
                className="min-h-[44px] px-4 py-2.5 rounded-xl bg-rose-50 text-rose-700 border border-rose-200 hover:bg-rose-100 text-xs font-bold transition-all flex items-center gap-1.5 active:scale-95"
              >
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                onClick={onClose}
                disabled={isSubmitting}
                className="min-h-[44px] px-4 py-2.5 rounded-xl bg-slate-100 text-slate-700 border border-slate-200 hover:bg-slate-200 text-xs font-bold transition-all active:scale-95"
              >
                Annulla
              </button>
              <button
                ...
                className="min-h-[44px] px-5 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white font-bold disabled:opacity-50 text-xs transition-all shadow-md shadow-indigo-600/20 flex items-center gap-1.5 active:scale-95"
              >
                {isSubmitting ? (
                  <>
