...
export default function TaskModal({ isOpen, onClose, taskToEdit = null, initialDate = '', onSave, onDelete }) {
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-end sm:items-center justify-center bg-black/75 backdrop-blur-md transition-opacity">
      ...
      {/* Soft Dark Surface Container */}
      <div className="relative w-full max-w-lg bg-[#232730] border-t sm:border border-white/[0.08] rounded-t-3xl sm:rounded-2xl shadow-2xl shadow-black/50 overflow-hidden text-slate-100 max-h-[85dvh] sm:max-h-[90vh] flex flex-col z-10 animate-slide-up-sheet sm:animate-none">
        ...
        {/* Header Modal */}
        <div className="flex items-center justify-between px-5 py-3.5 border-b border-white/[0.06] bg-[#1d2026]">
          <h2 className="text-xs font-semibold tracking-wider uppercase text-slate-200 flex items-center gap-2">
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4 text-xs overflow-y-auto flex-1">
          ...
          <div>
            ...
            <input
              ...
              className="w-full px-3.5 py-3 rounded-xl bg-[#191c22] border border-white/[0.08] text-white placeholder-slate-500 focus:outline-none focus:border-indigo-500 transition-all text-xs min-h-[44px]"
            />
          </div>
              ...
              <button
                ...
                className={`min-h-[44px] py-2 px-2.5 rounded-xl border font-medium text-[11px] transition-all flex items-center justify-center gap-1.5 touch-manipulation active:scale-95 ${
                  type === 'event'
                    ...
                    : 'bg-[#191c22] text-slate-400 border-white/[0.06] hover:border-slate-500'
                }`}
              >
          ...
          <div className="relative">
            ...
            <button
              ...
              className="w-full min-h-[44px] px-3.5 py-2.5 rounded-xl bg-[#191c22] border border-white/[0.08] hover:border-slate-500 text-white transition-all text-xs flex items-center justify-between cursor-pointer"
            >
            ...
            {isCategoryDropdownOpen && (
              <>
                ...
                />
                <div className="absolute z-50 left-0 right-0 mt-1 py-1.5 bg-[#2a2f3a] border border-white/10 rounded-xl shadow-2xl space-y-0.5 max-h-56 overflow-y-auto animate-fade-in">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              <div>
                ...
                <input
                  ...
                  className="w-full min-h-[44px] px-3.5 py-2.5 rounded-xl bg-[#191c22] border border-white/[0.08] text-white focus:outline-none focus:border-indigo-500 transition-all text-xs font-mono"
                />
              </div>
            ...
            <div className="flex items-center gap-1.5 pt-1 overflow-x-auto no-scrollbar">
              ...
              {[
                ...
                <button
                  ...
                  className="min-h-[36px] px-3 py-1.5 rounded-lg bg-[#191c22] hover:bg-indigo-600/20 text-slate-300 hover:text-white border border-white/[0.08] text-[11px] font-semibold transition-all shrink-0 active:scale-95"
                >
                  {p.label}
                </button>
              ))}
          ...
          <div>
            ...
            <textarea
              ...
              className="w-full px-3.5 py-2.5 rounded-xl bg-[#191c22] border border-white/[0.08] text-white placeholder-slate-500 focus:outline-none focus:border-indigo-500 transition-all text-xs"
            />
          </div>

          {/* Pulsanti Azione */}
          <div className="flex items-center justify-between pt-4 border-t border-white/[0.06] mt-4 pb-safe">
            {taskToEdit ? (
              <button
                type="button"
                onClick={handleDelete}
                disabled={isSubmitting}
                className="min-h-[44px] px-4 py-2.5 rounded-xl bg-rose-950/30 text-rose-400 border border-rose-900/40 hover:bg-rose-900/30 text-xs font-semibold transition-all flex items-center gap-1.5 active:scale-95"
              >
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                onClick={onClose}
                disabled={isSubmitting}
                className="min-h-[44px] px-4 py-2.5 rounded-xl bg-[#191c22] text-slate-300 border border-white/[0.08] hover:bg-white/5 text-xs font-semibold transition-all active:scale-95"
              >
                Annulla
              </button>
              <button
                ...
                className="min-h-[44px] px-5 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white font-semibold disabled:opacity-50 text-xs transition-all shadow-md shadow-indigo-600/25 flex items-center gap-1.5 active:scale-95"
              >
                {isSubmitting ? (
                  <>
