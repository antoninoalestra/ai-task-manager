...
export default function AuthModal({ isOpen, onClose, onAuthSuccess }) {
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-end sm:items-center justify-center bg-black/75 backdrop-blur-md transition-opacity">
      ...
      <div className="relative w-full max-w-md bg-[#232730] border-t sm:border border-white/[0.08] rounded-t-3xl sm:rounded-2xl shadow-2xl shadow-black/50 overflow-hidden text-slate-100 max-h-[85dvh] sm:max-h-[90vh] flex flex-col z-10 animate-slide-up-sheet sm:animate-none">
        ...
        <div className="p-1 mx-5 mt-4 bg-[#191c22] border border-white/[0.06] rounded-xl flex items-center shrink-0">
          <button
            type="button"
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4 text-xs overflow-y-auto flex-1">
          {mode === 'register' && (
            <div>
              ...
              <input
                ...
                className="w-full min-h-[44px] px-3.5 py-2.5 rounded-xl bg-[#191c22] border border-white/[0.08] text-white placeholder-slate-500 focus:outline-none focus:border-indigo-500 transition-all text-xs"
              />
            </div>
          )}
          ...
          <div className="pb-safe pt-2">
            <button
              type="submit"
              disabled={loading}
              className="w-full min-h-[44px] py-3 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white font-semibold disabled:opacity-50 text-xs transition-all shadow-md shadow-indigo-600/25 flex items-center justify-center gap-2 active:scale-95"
            >
